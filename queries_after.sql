...
SELECT mls.site_name,
       mls.base_domain,
       SUM(mls.total_requests)  AS total_requests,
       SUM(mls.total_bytes)     AS total_bytes,
       AVG(mls.avg_response_ms) AS avg_response_ms
FROM mv_link_stats mls
WHERE mls.last_request_at >= now() - interval '30 days'
   OR mls.last_request_at IS NULL
GROUP BY mls.target_site_id, mls.site_name, mls.base_domain
